...
# Member A or B (management-account AWS creds; no leftover AWS_SESSION_TOKEN):
...
Omit the model name to hit every marketplace alias (sync + stream). Curl uses the API key only; `ACCOUNT=a` assumes the member role in a subshell just to read the Function URL.
